<?php
    ...
    // Fetch data
    $sql = "SELECT * FROM images ORDER BY id ASC LIMIT $start_from, $limit";
    $result = $conn->query($sql);

    // Total records count
    $count_sql = "SELECT COUNT(*) as total FROM images";
    ...
    <div class="card shadow-lg">
        <div class="card-header text-white">
            <div class="d-flex justify-content-between align-items-center">
                <h3 class="mb-0"><i class="fas fa-users-cog me-2"></i>Images Management</h3>
                <div class="d-flex align-items-center">
                    <button class="btn btn-success ms-3 add-new" onclick="location.href='?e=images'">
        ...
        <div class="card-body">
                ...
                <div class="table-responsive table-container mb-4">
                    <table class="table table-hover user-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><img src="../assets/img/a_rahman/<?= htmlspecialchars($row["image_url"]) ?>" width="50"></td>
                                <td><?= htmlspecialchars($row["title"]) ?></td>
                                <td class="text-center action-buttons">
                                    <button class="btn btn-sm btn-edit me-1" title="Edit" onclick="location.href='?e=images&id=<?= encryptSt($row['id']) ?>'">
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination mb-0">
                            ...
                            <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                                <a class="page-link" href="?q=images&page=1<?= !empty($search) ? '&search='.urlencode($search) : '' ?>" aria-label="First">
                            ...
                            <!-- Previous Page -->
                            <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                                <a class="page-link" href="?q=images&page=<?= $page-1 ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>" aria-label="Previous">
                            ...
                            <?php for ($i = $start_range; $i <= $end_range; $i++): ?>
                                <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                    <a class="page-link" href="?q=images&page=<?= $i ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>"><?= $i ?></a>
                                </li>
                            <?php endfor; ?>
                            ...
                            <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : '' ?>">
                                <a class="page-link" href="?q=images&page=<?= $page+1 ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>" aria-label="Next">
                            ...
                            <!-- Last Page -->
                            <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : '' ?>">
                                <a class="page-link" href="?q=images&page=<?= $total_pages ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>" aria-label="Last">
            ...
            <?php else: ?>
                <div class="text-center py-5">
                    ...
                    <?php else: ?>
                        <button class="btn btn-primary mt-2 add-new" onclick="location.href='?e=images'">
                            <i class="fas fa-plus me-1"></i> Add New
                        </button>
                    <?php endif; ?>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('btnConfirmDelete').addEventListener('click', function() {
            ...
            fetch('api/delete.php', {
                ...
                body: JSON.stringify({ id: idToDelete, table: 'images' })
            })
